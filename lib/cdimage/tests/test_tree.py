    ...
    def test_mark_current_trigger(self, mock_polish_directory,
        ...
        if pid == 0:  # child
            ...
            with open(os.path.join(publish_base, "20130321", ".marked_good"),
                      "r") as marked_good:
                self.assertEqual("groovy-desktop-i386.iso\n",
                                 marked_good.read())
...
class TestDailyTreePublisher(TestCase):
    ...
    def test_size_limit(self):
        for project, dist, image_type, arch, size_limit in (
            ...
            ("ubuntu", "groovy", "daily-live", "amd64", 3221225472),
            ("ubuntukylin", "groovy", "daily-live", "amd64", 4294967296),
            ...
            ("ubuntu-server", "bionic", "daily", "ppc64el", 1200000000),
        ):
            if dist is not None:
                self.config["DIST"] = dist
