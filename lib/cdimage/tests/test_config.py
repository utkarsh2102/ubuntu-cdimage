...
class TestConfig(TestCase):
    ...
    def test_livefs_project_for_arch(self):
        config = Config(read=False)
    ...
    def test_livefs_arch_for_arch(self):
        config = Config(read=False)
        config.livefs_arch_mapping = {"amd64": ("ubuntu-cpc", "amd64+generic")}
